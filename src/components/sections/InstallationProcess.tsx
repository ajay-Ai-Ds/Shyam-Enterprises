      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 flex flex-col gap-3">
          ...
          <p className="text-slate-500 text-sm leading-relaxed">
            From initial site consultation to professional anchoring, here is how we ensure top-tier safety for your Bangalore home.
          </p>
        </div>

        {/* Steps Timeline Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative">
          {/* Burgundy Connector Line (Desktop Only) */}
          <div className="hidden lg:block absolute top-[32px] left-[12%] right-[12%] h-1 bg-primary-700 -z-0 rounded-full"></div>
          ...
          {steps.map((step, index) => (
            ...
            >
              {/* Step Gold Circle */}
              <div className="w-16 h-16 rounded-2xl bg-accent-orange group-hover:bg-accent-hover text-white flex items-center justify-center shadow-lg transition-colors duration-300 mb-6 shrink-0 relative">
                {step.icon}
                {/* Step Burgundy micro-badge */}
                <div className="absolute -top-1 -right-1 bg-primary-700 group-hover:bg-primary-800 text-white text-[10px] font-extrabold w-5 h-5 rounded-full flex items-center justify-center border-2 border-white transition-colors duration-300">
                  {step.id}
                </div>
              </div>

              {/* Title & Description */}
              <h3 className="text-base font-bold text-primary-700 mb-2.5 group-hover:text-accent-orange transition-colors">
                {step.title}
              </h3>
