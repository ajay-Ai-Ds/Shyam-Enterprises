...
export default function FloatingCTA() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-3 md:bottom-8 md:right-8 items-end">
      {/* Scroll to Top Button (Shows when scrolled > 200px) */}
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            className="flex items-center justify-center w-12 h-12 bg-slate-800 text-white rounded-full shadow-xl hover:bg-slate-700 transition-colors duration-300 relative group cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-slate-800 focus:ring-offset-2"
          >
      ...
      {/* Direct Call Button (Safety Burgundy / Orange Pulse) — Visible from Start */}
      <motion.a
        ...
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.95 }}
        aria-label="Call Shyam Enterprises"
        className="flex items-center justify-center w-13 h-13 bg-accent-orange text-white rounded-full shadow-2xl hover:bg-accent-hover transition-all duration-300 relative group focus:outline-hidden focus:ring-2 focus:ring-accent-orange focus:ring-offset-2 cursor-pointer"
      >
        <Phone className="w-6 h-6" />
        <span className="absolute right-16 bg-slate-900 text-white text-xs font-semibold px-3 py-1.5 rounded-md shadow-lg opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap">
          Call +91 95139 89222
        </span>
        <span className="absolute inset-0 w-full h-full rounded-full bg-accent-orange animate-ping opacity-30 -z-10 group-hover:hidden"></span>
      </motion.a>

      {/* WhatsApp Button (WhatsApp Green) — Visible from Start */}
      <motion.a
        ...
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.95 }}
        aria-label="Chat on WhatsApp with Shyam Enterprises"
        className="flex items-center justify-center w-13 h-13 bg-[#25D366] text-white rounded-full shadow-2xl hover:bg-emerald-600 transition-all duration-300 relative group focus:outline-hidden focus:ring-2 focus:ring-[#25D366] focus:ring-offset-2 cursor-pointer"
      >
